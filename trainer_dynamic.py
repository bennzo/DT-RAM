...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            msg2 = "- val loss: {:.3f} - val acc: {:.3f} - val err: {:.3f} - valid glimpses: {:.3f}"
            if is_best:
                self.counter = 0
                msg2 += " [*]"
            msg = msg1 + msg2
    ...
    def train_one_epoch(self, epoch):
        ...
        with tqdm(total=self.num_train) as pbar:
            for i, (x, y) in enumerate(self.train_loader):
                ...
                log_pi_stops = torch.stack(log_pi_stop).transpose(1, 0)         # (N, steps)
                ...
                loss_reinforce = -log_pi_loc[:, first_correct_step] * log_pi_stops[:, first_correct_step] * R
                ...
                # compute accuracy
                correct = (predictions_per_step[:, first_correct_step] == y).float()
                ...
                glimpses.update(torch.mean(first_correct_step), torch.sum(R))
            ...
            return losses.avg, accs.avg, glimpse.avg
